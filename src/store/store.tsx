...
import type { FollowUpDraft, HistoryEntry, Pipeline, PrepOutput, Profile, Stage } from '../types'
...
// v4: pipeline shape gained followUpDraft.
const PIPELINES_KEY = 'ghosted:pipelines:v4'
const PROFILE_KEY = 'ghosted:profile:v2'
const SEEDED_KEY = 'ghosted:seeded:v4'
...
interface StoreValue {
  ...
  generateFollowUpFor: (id: string) => FollowUpDraft | null
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const generateFollowUpFor: StoreValue['generateFollowUpFor'] = useCallback(
    (id) => {
      const pipeline = pipelines.find((p) => p.id === id)
      if (!pipeline) return null
      const followUpDraft = generateFollowUp(pipeline, profile)
      setPipelines((prev) => prev.map((p) => (p.id === id ? { ...p, followUpDraft } : p)))
